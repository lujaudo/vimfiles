...
fun! s:RgGetVisualSelection()
    ...
    let l:searchStr = join(lines, "\n")
    call mark#DoMark(v:count,l:searchStr)
    return l:searchStr
endfun
...
fun! s:RgSearch(txt)
  ...
  silent! exe 'grep! ' . l:rgopts . a:txt . ' ' . g:rg_path
  if len(getqflist())
...
command! -range Rgv :call g:RgVisual()
command! -complete=file RgRoot :call s:RgShowRoot()

nmap <unique> <Leader>g :normal <leader>m<CR>:Rg<CR>
vmap <unique> <Leader>g :normal <C-u>call g:RgVisual()<CR>
